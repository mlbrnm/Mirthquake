...
class Message:
	...
	def __init__( self, text=None ):
		...
		Arguments:
		text -- Optional text block to use as the message content. If provided, it will be used as a single message.
		"""
		self.Lines = []
		if text is not None:
			self.SetText(text)
	...
	def SetText( self, text ):
		"""
		Set the entire message content as a single text block.
		
		Arguments:
		text -- The text block to use as the message content
		
		Example:
		SetText("<Rec CRC='D90F'><Ver>1</Ver>...")
		"""
		self.Lines = [Line(text)]

	def Render( self ):
		""" 
		Render all lines in this message with required null bytes:
		- Two null bytes at the start
		- One null byte at the end
		"""
		output = ''
		for thisLine in self.Lines:
			output += thisLine.Render()
		return NUL + NUL + output + NUL
...
class Connection:
	...
	def Send( self, msg ):
		...
		Returns:
		bytes -- The raw response from the server
		"""
